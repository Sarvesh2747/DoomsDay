...
export default function Home() {
  ...
  if (showAgeGate) {
    return <AgeGate onConfirm={() => setShowAgeGate(false)} />;
  }
...
function ScrubImage({ 
  ...
  progress: MotionValue<number>; src: string; index: number; total: number; start: number; end: number; 
}) {
  ...
  return (
    ...
    >
      ...
      <nav className="fixed top-0 w-full z-50 px-8 pt-8 pb-24 flex justify-between items-start bg-gradient-to-b from-black/90 via-black/40 to-transparent pointer-events-auto">
        <div className="text-3xl font-black tracking-widest text-white cursor-pointer hover:text-[#ffb8e0] transition-colors drop-shadow-lg">
          R★
        </div>
        <div className="hidden md:flex gap-10 text-xs font-bold tracking-[0.2em] uppercase text-white/90 pt-2">
          <a href="#" className="hover:text-white transition-colors drop-shadow-md">Analysis</a>
        ...
        <button className="px-6 py-3 bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest rounded-sm hover:bg-yellow-300 transition-colors shadow-[0_0_20px_rgba(250,204,21,0.4)]">
          Analyze Now
        </button>
      </nav>
      ...
      <motion.section
        className="fixed top-0 left-0 w-full h-screen flex flex-col items-center justify-end z-40 overflow-hidden pointer-events-none"
        style={{ opacity: creditsOpacity }}
      >
        {/* Art background: Vice City beach sunset */}
        <img
          src="https://www.rockstargames.com/VI/_next/image?url=%2FVI%2F_next%2Fstatic%2Fmedia%2FHero_BG.d0b73d7a.jpg&w=1920&q=100"
        ...
        {/* Content */}
        <div className="relative z-10 w-full max-w-5xl mx-auto px-8 pb-16 flex flex-col items-center gap-10">
          ...
          <p className="text-white/20 text-[10px] tracking-[0.4em] uppercase">
            Built with Next.js · Framer Motion · Graph Theory · 2025
          </p>
        </div>
      </motion.section>
    ...
    <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
      ...
      >
        ...
        <div className="flex gap-4">
          <button 
            onClick={onConfirm}
            className="flex-1 py-5 bg-white text-black uppercase font-black tracking-widest rounded-sm hover:bg-[#e0e0e0] transition-colors"
          >
            I am 18+
          </button>
        </div>
